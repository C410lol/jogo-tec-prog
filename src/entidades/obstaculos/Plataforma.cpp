    ...
    namespace entidades {
        namespace obstaculos {
            ...
            Plataforma::Plataforma(sf::Vector2f r_posicao, sf::Vector2f r_tamanho, bool r_danoso, bool r_ehChao,
                                   IDs id) : Obstaculo(r_posicao, r_tamanho, r_danoso), ehChao(r_ehChao),
                                             invisivel(false)
            {
                if (!ehChao && pFase->getId() == IDs::primeira_fase) {
                    ...
                    ++numInstancias;
                } else if (ehChao &&pFase
                
                ->
                getId() == IDs::primeira_fase
                )
            ...
            Plataforma::Plataforma(dtos::ObstaculoDTO obsDTO, bool r_ehChao, bool r_invisivel) : Obstaculo(obsDTO),
                ehChao(r_ehChao), invisivel(r_invisivel)
            {
                if (!ehChao && pFase->getId() == IDs::primeira_fase) {
                    setTexture("../assets/obstaculos/plataformaFase1.png");
                    ++numInstancias;
