...
namespace jogo {
    namespace fases {
        SegundaFase::SegundaFase(int r_numJogadores, IDs id, bool continuar) : Fase(r_numJogadores), maxTerrestres(8),
                                                                               maxChefoes(7), maxPlataformas(325),
                                                                               maxMelecas(15)
        {
        ...
        void SegundaFase::criarObstaculos(char c, float x, float y)
        {
            switch (c) {
                ...
                case '5':
                    if (rand() % 2)
                        criarMelecas(sf::Vector2f(x, y + proporcao.y / 1.25),
